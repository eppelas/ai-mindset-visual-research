...
export const DesktopTechUiV5 = () => {
  ...
  return (
    <div className="w-full max-w-[1340px] mx-auto py-12 px-8 font-sans">
      <div className="flex flex-col lg:flex-row gap-20 items-stretch justify-center">
        
        {/* LEFT: STEPPER NAVIGATION (ALIGNED TO CARD CONTENT) */}
        <div className="w-[124px] shrink-0 flex flex-col py-[50px] justify-between h-[580px]">
          {PROGRAM_TRACKS.map((t, idx) => {
            ...
            return (
              <button
                key={`v5-st-ref-f-final-al-v14-${t.id}`}
                onClick={() => setActiveWeek(idx)}
                className="flex items-center gap-5 group text-left relative h-12"
              >
                {idx < PROGRAM_TRACKS.length - 1 && (
                  <div className="absolute left-[20px] top-[40px] w-[1px] h-[calc(550px/3)] bg-black/[0.08]" />
                )}
                
                <div className={cn(
                  "w-10 h-10 rounded-full border flex items-center justify-center transition-all z-10 shrink-0 shadow-sm",
                  isActive ? "bg-[#8DC63F] border-[#8DC63F] shadow-[#8DC63F]/20" : "bg-white border-black/[0.1] group-hover:border-black/20"
                )}>
        ...
        {/* RIGHT: THE MAIN CARD */}
        <div className="flex-1 bg-white border border-black/15 h-[580px] shadow-[0_10px_40px_rgba(0,0,0,0.02)] relative overflow-hidden flex flex-col rounded-[2px] pt-12 max-w-[940px]">
          ...
          <div className="relative z-20 flex flex-col flex-1 px-12 pb-12">
             ...
             <div className="flex-1 flex flex-col lg:flex-row gap-8 relative overflow-hidden">
                
                {/* Content Area - REDUCED BLUR (8px) */}
                <div className="flex-1 min-w-0 relative">
                  <AnimatePresence>
                    <motion.div
                      key={`v5-blur-crossfade-reduced-${activeWeek}`}
                      initial={{ opacity: 0, filter: "blur(8px)" }} // Reduced blur from 20px
                      animate={{ opacity: 1, filter: "blur(0px)" }}
                      exit={{ opacity: 0, filter: "blur(8px)" }}
                      transition={{ duration: 0.5, ease: [0.22, 1, 0.36, 1] }} // Slightly snappier duration
                    ...
                    >
                      ...
                      <div className="mt-auto">
                        ...
                        <div className="grid grid-cols-7 border border-black/[0.1] w-full max-w-[720px] bg-black/[0.05] gap-px rounded-[1px] overflow-hidden shadow-none">
                          {weeklyRhythm.map((item, idx) => (
                            <div key={`v5-cal-mod-final-v6-al-f-v2-al-v2-${idx}`} 
                              className={cn(
                            ...
                            >
                               ...
                               <div className={cn("text-[10px] font-black uppercase mt-auto leading-none tracking-tighter font-mono", 
                ...
                {/* Advanced Info Area - REDUCED BLUR */}
                <div className="w-full lg:w-[320px] shrink-0 relative pt-8">
                  <AnimatePresence>
                    <motion.div
                      key={`v5-restore-adv-crossfade-reduced-${activeWeek}`}
                    ...
                    >
                      <div className="bg-gradient-to-l from-gray-100/100 via-gray-100/60 via-gray-50/20 to-transparent p-10 py-12 flex-col items-end justify-start w-full backdrop-blur-[1px] flex min-h-[220px]">
